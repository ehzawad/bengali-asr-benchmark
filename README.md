...
A controlled comparison of public Bengali speech-to-text models: six models on the FLEURS
Bengali test split, and five of them on the official test splits of three other Bengali
corpora — Vaani; SPRING-INX R1 and R2; Kathbath known- and unknown-speaker (five test sets in
all). Every decode was run on one GPU (RTX A5000), one model at a time, through the same audio
path and the same scorer — with one exception, marked † below — so accuracy and speed are
comparable across the tables.
...
- **`ehzawad/stt_bn_fastconformer_ctc`** — the most accurate model on the corpora none of these
  ...
  on FLEURS, at CTC latency (~83 ms per clip).
- **`hishab/titu_stt_bn_conformer_large`** — best on FLEURS (14.4 %) and on Kathbath, i.e. on read
  ...
  transfer to spontaneous Indian-district speech.
...
- **The Qwen3 adapter** — statistically tied with `ehzawad/stt_bn_fastconformer_ctc` on FLEURS, at
  ~9.7 s per clip (117× slower). Only if you need exactly that accuracy and can afford the time.
...
## Official test splits of three other Bengali corpora
...
five the card does not settle it either way. Kathbath (AI4Bharat IndicSUPERB, read speech) is a
corpus two of the models trained another split of.
...
under [`outputs_multiset/`](outputs_multiset/). The Qwen3 adapter is not in this comparison — at
~10 s per clip it was impractical over 5,000 more utterances.

**WER on the 1,000-utterance official-test subsets** (best per column in bold; 95 % intervals in brackets):

| Model | Vaani test | SPRING-INX R1 eval | SPRING-INX R2 eval | Kathbath (known spk) | Kathbath (unknown spk) |
|---|---:|---:|---:|---:|---:|
...
**S** — the card says the model trained on another split of this corpus; **U** — the card does
not say either way. None of the three means "verified clean".
...
- The FLEURS ranking does not transfer. Conformer Large, the FLEURS leader at 14.4 %, is last on
  ...
  SUBAK.KO, Shrutilipi, Vasha-Bichita, IndicTTS) and neither Vaani nor SPRING-INX.
...
| Qwen3-ASR + Bengali adapter | `ehzawad/stt_bn_qwen3_asr` on `Qwen/Qwen3-ASR-1.7B-hf` | 16.54% [15.43–17.68] | 4.68% | 9,728 | 2.04B (38M LoRA) |
...
**Test split only.** The two `ehzawad` models (the FastConformer-CTC and the Qwen3 adapter)
trained on corpora that include the FLEURS *train* split, which overlaps the FLEURS
*validation* split, so the 402 validation recordings are excluded for every model. The
FastConformer-CTC had also been evaluated on FLEURS test during its own development, so read
its FLEURS row as a like-for-like re-measurement and the other corpora above as the fresh evidence.

Paired comparisons for `ehzawad/stt_bn_fastconformer_ctc` on this split:

- Statistically tied with the Qwen3 adapter — +0.58 points, interval [−0.25, +1.40] spans zero — at 117× its speed.
- +0.89 points behind Whisper Medium [+0.27, +1.50], at 22× its speed.
- +2.74 points behind Conformer Large [+2.01, +3.46].
...
- **One audio path.** `asr_core.py` loads, downmixes and resamples to 16 kHz and hands identical
  bytes to every backend — NeMo, wav2vec2, Whisper and Qwen all read the same audio.
- **One scorer.** `bench_score.norm` strips punctuation to word boundaries and applies Unicode NFC
  ...
  base + nukta; the FLEURS references and the NeMo/Qwen models use the decomposed form, Whisper
  and wav2vec2 the precomposed one, and without NFC every such character scores as a substitution
...
- **Greedy decoding, batch 1, no external LM** for every decode made here — the interactive-latency
  workload, applied identically to CTC and autoregressive decoders. Batching would help the
  autoregressive models far more; it is not measured here for anyone.
- **Every clip counts.** No model raised on a clip. Two models emitted a handful of empty
  hypotheses on SPRING R2 (2 and 1); those clips stay in the denominator as full errors.
- **Checkpoints pinned** by commit hash in `outputs_fleurs/checkpoint_revisions.json`; the Qwen3
  adapter was loaded from a local copy of `ehzawad/stt_bn_qwen3_asr` on the pinned base revision.
...
- FLEURS and Kathbath are read speech; Vaani and SPRING are spontaneous. No telephony, no
  far-field, no noise conditions.
...
- Cell intervals on the 1,000-utterance subsets run from about ±0.7 points (Kathbath) to ±1.2
  points (Vaani, SPRING), with one cell at ±2.5 points. Differences inside an interval are not
  differences.
...
export QWEN_ADAPTER=<local copy of ehzawad/stt_bn_qwen3_asr>     # needs its own interpreter: BENCH_PY_QWEN
BENCH_GPU=<A5000 uuid> ./run_all_fleurs.sh        # six models on FLEURS, one at a time -> outputs_fleurs/
...
The Qwen3 adapter needs a second interpreter (`BENCH_PY_QWEN`) because its transformers version
differs from NeMo's. Subset sampling: seed 20260915, proportional stratification (Vaani by
state/district, SPRING by Latin/no-Latin reference, Kathbath by speaker gender). The Vaani,
SPRING-INX and Kathbath audio is not redistributed here; `multiset/*/eval_manifest.json` lists
every selected utterance with its SHA-256 so a rebuild can be verified.
...
- Evaluation data: [FLEURS](https://huggingface.co/datasets/google/fleurs) (Google), CC-BY-4.0;
  ...
  release page. The committed manifests and predictions carry reference transcripts alongside each
  model's hypothesis.
...
- `ehzawad/stt_bn_fastconformer_ctc` — FastConformer-CTC large trained on 1,692 hours of
  ...
  `nvidia/stt_en_fastconformer_ctc_large`, NVIDIA, CC-BY-4.0. `ehzawad/stt_bn_qwen3_asr` — Bengali
  LoRA adapter on `Qwen/Qwen3-ASR-1.7B-hf`.
