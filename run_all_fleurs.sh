#!/usr/bin/env bash
# Five models, one GPU (the A5000, by UUID), one model resident at a time, the
...
# The local checkpoint is taken from FASTCONFORMER_NEMO
# (env), the interpreter from BENCH_PY.
set -u
cd "$(dirname "$0")"
...
say "five models on the FLEURS eval set, one at a time"
