...
#   * Phase 4 (stt_bn_fastconformer_ctc, step_116000, sha bea300ce…) was
...
#   * This is the THIRD time the Phase-4 lineage decodes FLEURS-bn test
#     (events one and two are registered in SPEC 8w.14 / 8w.18). FLEURS is
#     therefore not a held-out set for Phase 4 in this run; these numbers are a
...
# Outputs go under outputs/<label>_p4repro so the published outputs/<label>
...
say "PROSPECTIVE: post-release reproducibility run; Phase 4 = third FLEURS exposure; no selection from these numbers"
run phase4_fastconformer_ctc nemo     "$P/experiments/p4/runs/N_O+macro_filtered_H120k/export/step_116000.nemo"
